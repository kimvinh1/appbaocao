  ...
  return (
    <div className="mx-auto grid min-h-[80vh] max-w-5xl gap-6 lg:grid-cols-[1.15fr_0.85fr]">
      <section className="glass-panel rounded-[2rem] border border-cyan-500/15 p-8 sm:p-10">
        ...
        <h1 className="mt-4 text-4xl font-semibold text-white">Cổng điều phối hỗ trợ kỹ thuật</h1>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          <div className="rounded-2xl border border-slate-700/70 bg-slate-900/50 p-4">
            ...
            <p className="mt-3 text-sm font-semibold text-white">Phân quyền nội bộ</p>
            <p className="mt-1 text-sm text-slate-400">Admin tạo user và kiểm soát quyền truy cập.</p>
          </div>
          <div className="rounded-2xl border border-slate-700/70 bg-slate-900/50 p-4">
            <UserRoundPlus className="text-cyan-300" size={22} />
            <p className="mt-3 text-sm font-semibold text-white">Chia sẻ quy trình</p>
            <p className="mt-1 text-sm text-slate-400">Gửi link cho khách hàng và ghi nhận trạng thái hoàn tất.</p>
          </div>
          <div className="rounded-2xl border border-slate-700/70 bg-slate-900/50 p-4">
            <LockKeyhole className="text-red-400" size={22} />
            <p className="mt-3 text-sm font-semibold text-white">Case có ảnh</p>
            <p className="mt-1 text-sm text-slate-400">Lưu lại ảnh lỗi hoặc hiện trường để truy vết nhanh hơn.</p>
          </div>
        </div>
      </section>
      ...
      <section className="glass-panel rounded-[2rem] p-8 sm:p-10">
        {hasUsers ? (
          <>
            <p className="text-sm font-medium text-slate-400">Đăng nhập hệ thống</p>
            <h2 className="mt-2 text-2xl font-semibold text-white">Chào mừng quay lại</h2>
            <form action={loginAction} className="mt-8 space-y-4">
              <label className="block text-sm text-slate-300">
                ...
                <input
                  ...
                  type="email"
                  required
                  className="mt-1 w-full rounded-xl border border-slate-600 bg-slate-950/70 px-4 py-3 text-sm text-white outline-none transition focus:border-cyan-400"
                />
              </label>
              <label className="block text-sm text-slate-300">
                Mật khẩu
          ...
          <>
            <p className="text-sm font-medium text-slate-400">Khởi tạo hệ thống</p>
            <h2 className="mt-2 text-2xl font-semibold text-white">Tạo tài khoản admin đầu tiên</h2>
            <form action={bootstrapAdmin} className="mt-8 space-y-4">
              <label className="block text-sm text-slate-300">
                ...
                <input
                  name="fullName"
                  required
                  className="mt-1 w-full rounded-xl border border-slate-600 bg-slate-950/70 px-4 py-3 text-sm text-white outline-none transition focus:border-orange-400"
                />
              </label>
              <label className="block text-sm text-slate-300">
                Email
