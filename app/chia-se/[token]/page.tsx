    ...
    <div className="mx-auto max-w-5xl space-y-4 sm:space-y-6">
      ...
      <div className="glass-panel rounded-2xl px-4 py-4 sm:px-6">
        <ol className="flex items-center gap-0 text-xs text-slate-500 dark:text-slate-600">
          ...
          <li className="w-8 h-px bg-slate-200 dark:bg-slate-700 shrink-0" />
          {/* Bước 2 */}
          <li className="flex items-center gap-2 flex-1">
      ...
      {/* Nội dung bài viết */}
      <section className="glass-panel rounded-[2rem] p-4 sm:p-6 lg:p-10 mt-6 bg-white/50 dark:bg-slate-900/50">
      ...
      <section className="glass-panel rounded-2xl p-5 sm:p-6">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
          ...
          <span className={`rounded-full px-3 py-1 text-xs font-medium ${share.status === 'completed' ? 'bg-emerald-500/15 text-emerald-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300'}`}>
        ...
        {'customerComment' in share && share.customerComment && (
          <div className="mb-4 rounded-xl border border-slate-300/60 dark:border-slate-700/60 bg-slate-50/40 dark:bg-slate-950/40 p-4 text-sm text-slate-700 dark:text-slate-300 italic">
