...
export function ShareInteractionPanel({
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="text-sm text-slate-700 dark:text-slate-300 block mb-1.5">
          Nhận xét của bạn <span className="text-slate-600 dark:text-slate-500">(không bắt buộc)</span>
        </label>
        <textarea
          value={comment}
          ...
          className="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white/80 dark:bg-slate-900/80 px-3 py-2 text-sm text-white outline-none resize-none placeholder:text-slate-500 focus:border-slate-400 transition"
        />
      </div>
      {renderButtons()}
    </div>
