    ...
    <>
      ...
      <aside className="hidden xl:block fixed right-6 top-24 w-56 z-20">
        <div className="glass-panel rounded-xl px-4 py-3 text-sm">
          <p className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 dark:text-slate-600 uppercase tracking-wider mb-2">
              ...
              <a
                ...
                className={[
                  ...
                  activeId === h.id
                    ...
                    : 'text-slate-500 dark:text-slate-600 hover:text-slate-200 hover:bg-slate-800/60',
                ].join(' ')}
                title={h.text}
              >
                {h.text}
      ...
      <div className="xl:hidden glass-panel rounded-xl overflow-hidden">
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="w-full flex items-center justify-between gap-2 px-4 py-3 text-sm text-slate-700 dark:text-slate-300 hover:text-gray-900 dark:text-white transition"
        >
          <span className="flex items-center gap-1.5 font-medium">
            <List size={14} /> Mục lục ({headings.length} mục)
          </span>
          <span className={`text-slate-600 dark:text-slate-600 transition-transform ${open ? 'rotate-180' : ''}`}>▼</span>
        </button>
        {open && (
            ...
            {headings.map((h) => (
              <a
                ...
                className={[
                  ...
                  activeId === h.id ? 'text-cyan-300 font-medium' : 'text-slate-500 dark:text-slate-600 hover:text-slate-200',
                ].join(' ')}
              >
                {h.level === 2 ? '· ' : '  – '}{h.text}
              </a>
