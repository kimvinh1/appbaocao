    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <div className="glass-panel rounded-2xl px-8 py-14 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-600">Không tìm thấy mã lỗi phù hợp.</p>
          {hasFilter && (
              ...
              <summary className="p-4 flex cursor-pointer items-start justify-between gap-4 outline-none hover:bg-slate-800/40 transition list-none">
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-600 bg-slate-100/80 dark:bg-slate-800/80 px-2 py-0.5 rounded-full">{ec.instrument}</span>
                    <span className={`text-[12px] px-2 py-0.5 rounded-full font-medium ${SEVERITY_STYLE[ec.severity] ?? 'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300'}`}>
                      {SEVERITY_LABEL[ec.severity] ?? ec.severity}
                    </span>
                  </div>
                  <p className="text-sm font-medium text-gray-900 dark:text-white group-open:text-slate-300 transition-colors">{ec.description}</p>
                </div>
                <div className="mt-1 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-600 group-open:rotate-180 transition-transform">▼</div>
              </summary>
                ...
                <div className="space-y-4 border-t border-slate-300/40 dark:border-slate-700/40 pt-4">
                  ...
                  {ec.imageUrl && (
                    <div className="rounded-lg border border-slate-300/60 dark:border-slate-700/60 p-2 pl-3 pb-3">
                      <p className="text-xs font-semibold text-slate-500 dark:text-slate-600 mb-2 flex items-center gap-1">
