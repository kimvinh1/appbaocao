    ...
    <div className="space-y-4">
      ...
      <div className="glass-panel rounded-xl px-4 py-3 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-700 bg-slate-900/60 pl-8 pr-3 py-1.5 text-sm text-white placeholder:text-slate-500 outline-none focus:border-slate-500 transition"
          />
        </div>
        <select
          value={filterInstrument}
          onChange={(e) => setFilterInstrument(e.target.value)}
          className="rounded-lg border border-slate-700 bg-slate-900/60 px-3 py-1.5 text-sm text-white outline-none focus:border-slate-500 cursor-pointer"
        >
        ...
        {hasFilter && (
          <div className="flex items-center gap-2 text-xs text-slate-400">
            ...
            <button
              ...
              className="flex items-center gap-0.5 text-slate-500 hover:text-slate-300 transition"
              title="Xóa bộ lọc"
            >
              <X size={12} /> Xóa lọc
            </button>
      ...
      {filtered.length === 0 ? (
        <div className="glass-panel rounded-2xl px-8 py-14 text-center">
          ...
          <p className="text-slate-400">Không tìm thấy mã lỗi phù hợp.</p>
          {hasFilter && (
          ...
          {filtered.map((ec) => (
            <details key={ec.id} className="glass-panel rounded-xl group overflow-hidden">
              <summary className="p-4 flex cursor-pointer items-start justify-between gap-4 outline-none hover:bg-slate-800/40 transition list-none">
                ...
                <div className="mt-1 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-800 text-slate-400 group-open:rotate-180 transition-transform">▼</div>
              </summary>
