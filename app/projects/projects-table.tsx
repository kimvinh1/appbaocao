...
const INPUT_CLS = 'w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white/80 dark:bg-slate-900/80 px-3 py-2 text-sm text-white outline-none transition focus:border-cyan-400';
    ...
    <section className="space-y-3">
      ...
      <div className="glass-panel rounded-2xl p-4">
        <div className="flex flex-wrap items-center gap-2 mb-3">
          <Search size={14} className="text-slate-500 dark:text-slate-600" />
          <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Lọc dự án</span>
          {hasFilter && (
            <button onClick={clearFilters} className="ml-auto flex items-center gap-1 text-xs text-slate-500 dark:text-slate-600 hover:text-gray-900 dark:text-white transition">
        ...
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          ...
          <div className="flex flex-col gap-1">
            <span className="text-xs text-slate-600 dark:text-slate-600">Từ ngày</span>
            <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)}
              className={INPUT_CLS} />
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-xs text-slate-600 dark:text-slate-600">Đến ngày</span>
      ...
      <div className="table-shell">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {filtered.map((project) => {
              ...
              return (
                <tr key={project.id} className="border-t border-slate-200/80 dark:border-slate-800/80 hover:bg-slate-800/30">
                  ...
                  <td className="px-4 py-3 text-slate-500 dark:text-slate-600 text-xs">
                  ...
                  <td className="px-4 py-3 text-xs text-slate-500 dark:text-slate-600 max-w-[180px]">
                    {lastLog ? (
                      <div>
                        ...
                        <br />
                        <span className="text-slate-600 dark:text-slate-600 italic line-clamp-1">{lastLog.note}</span>
                      </div>
                    ) : <span className="text-slate-600 italic">Chưa có log</span>}
                  </td>
                  <td className="px-4 py-3">
                    <Link href={`/projects/${project.id}`}
                      className="rounded-lg bg-slate-200 dark:bg-slate-700 px-3 py-1.5 text-xs font-medium text-slate-800 dark:text-slate-200 transition hover:bg-cyan-500 hover:text-slate-950 whitespace-nowrap">
                      Chi tiết →
                    </Link>
                  </td>
                </tr>
            ...
            {!filtered.length && (
              <tr>
                <td colSpan={10} className="px-4 py-10 text-center text-sm text-slate-500 dark:text-slate-600">
