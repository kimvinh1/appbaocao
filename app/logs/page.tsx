  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">Ghi nhận khối lượng công việc hằng ngày và liên kết với dự án.</p>
      </div>
          ...
          <tbody>
            {logs.map((log) => (
              <tr key={log.id} className="border-t border-slate-800/80">
                <td className="px-4 py-3 text-slate-300">{new Date(log.logDate).toLocaleDateString()}</td>
                <td className="px-4 py-3 text-white">{log.teamMember}</td>
                <td className="px-4 py-3 text-slate-300">{LOG_CATEGORIES.find((category) => category.value === log.category)?.label ?? log.category}</td>
                <td className="px-4 py-3 text-slate-300">{log.durationHours.toFixed(1)}</td>
                <td className="px-4 py-3 text-slate-300">{log.project?.code ?? 'Không có'}</td>
