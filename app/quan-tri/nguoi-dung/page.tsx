        ...
        <form action={createUserAction} className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <label className="text-sm text-slate-300">
            ...
            <input
              name="fullName"
              required
              className="mt-1 w-full rounded-lg border border-slate-600 bg-slate-900/80 px-3 py-2 text-sm text-white outline-none transition focus:border-orange-400"
            />
          </label>
          ...
          <tbody>
            {users.map((user) => (
              <tr key={user.id} className="border-t border-slate-800/80">
                ...
                <td className="px-4 py-3 text-slate-300">{user.role === 'admin' ? 'Admin' : 'Nhân viên'}</td>
                <td className="px-4 py-3 text-slate-300">{user.isActive ? 'Đang hoạt động' : 'Ngưng hoạt động'}</td>
                <td className="px-4 py-3 text-slate-300">{new Date(user.createdAt).toLocaleDateString('vi-VN')}</td>
              </tr>
            ))}
          </tbody>
        </table>
