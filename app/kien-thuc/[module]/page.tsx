    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <Link href="/kien-thuc" className="flex items-center gap-1 text-xs text-slate-600 dark:text-slate-500 dark:text-slate-400 hover:text-white mb-2 transition">
                        <ArrowLeft size={12} /> Thư Viện Tài Liệu
                    </Link>
                    <h2 className={`text-2xl font-semibold ${cfg.textClass}`}>{cfg.title}</h2>
                    <p className="mt-1 text-sm text-slate-600 dark:text-slate-500 dark:text-slate-400">{cfg.description}</p>
                </div>
                <Link
            ...
            <section className="space-y-4" id="tai-lieu">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-600 dark:text-slate-500 dark:text-slate-400">Soạn SOP, hướng dẫn và case thực tế để chia sẻ theo từng khách hàng.</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                        <span className="inline-flex items-center gap-1 rounded-full bg-white/70 dark:bg-slate-900/70 px-3 py-1.5 text-xs text-slate-600 dark:text-slate-500 dark:text-slate-400 ring-1 ring-slate-200 dark:ring-slate-800">
                            ...
                            return (
                                <Link
                                    ...
                                    className={`rounded-full px-3 py-1.5 text-xs ring-1 transition ${
                                        active
                                            ? `${cfg.buttonClass}`
                                            : 'bg-white/70 dark:bg-slate-900/70 text-slate-700 dark:text-slate-300 ring-slate-200 dark:ring-slate-800 hover:bg-slate-800'
                                    }`}
                                >
                                    {item.label}
                                </Link>
                ...
                {filteredArticles.length === 0 ? (
                    <div className="glass-panel rounded-2xl px-8 py-16 text-center">
                        ...
                        <p className="text-slate-600 dark:text-slate-500 dark:text-slate-400">Chưa có nội dung nào đúng bộ lọc này.</p>
                    </div>
                ) : (
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {tags.map((tag) => (
                                            <span key={tag} className="flex items-center gap-1 text-xs text-slate-600 dark:text-slate-500 dark:text-slate-400 bg-slate-100/80 dark:bg-slate-800/80 px-2 py-0.5 rounded-full">
