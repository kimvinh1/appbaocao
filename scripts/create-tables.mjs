// Script tạo bảng trực tiếp trên Turso (không cần Prisma engine)
...
if (!url || !authToken) {
  console.error('❌ Thiếu DATABASE_URL hoặc DATABASE_AUTH_TOKEN');
  process.exit(1);
}
...
const SQL_STATEMENTS = [
  ...
  `CREATE TABLE IF NOT EXISTS "SupportCaseImage" (
    ...
    CONSTRAINT "SupportCaseImage_supportCaseId_fkey" FOREIGN KEY ("supportCaseId") REFERENCES "SupportCase"("id") ON DELETE CASCADE ON UPDATE CASCADE
  )`,
  ...
  `CREATE TABLE IF NOT EXISTS "ProcedureShare" (
    ...
    CONSTRAINT "ProcedureShare_sharedById_fkey" FOREIGN KEY ("sharedById") REFERENCES "User"("id") ON DELETE SET NULL ON UPDATE CASCADE
  )`,
  ...
  `CREATE TABLE IF NOT EXISTS "ProcedureReaction" (
    ...
    CONSTRAINT "ProcedureReaction_shareId_fkey" FOREIGN KEY ("shareId") REFERENCES "ProcedureShare"("id") ON DELETE CASCADE ON UPDATE CASCADE
  )`,
...
async function main() {
  ...
  console.log('\n🎉 Xong! Tất cả bảng đã được tạo trên Turso.');

  // Verify tables exist
  const result = await client.execute("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name");
  console.log('\n📋 Bảng hiện có:', result.rows.map(r => r.name).join(', '));
}
